...
class DPMLM():
    ...
    def dpmlm_rewrite_nouns(self, sentence, epsilon, REPLACE=False, FILTER=False, STOP=False, TEMP=True, POS=True, CONCAT=True):
        if isinstance(sentence, list):
            tokens = sentence
        else:
            tokens = nltk.word_tokenize(sentence)
        ...
        for i, (t, nn, eps) in enumerate(zip(tokens, n, word_eps)):
            ...
            if pos_tags[i][1] not in noun_pos_tags:
                total += 1
                replace.append(t)
                continue
    ...
    def dpmlm_rewrite_noun_phrase(self, sentence, epsilon, replace_dict={}, REPLACE=False, FILTER=False, STOP=False, TEMP=True, POS=True, CONCAT=True):
        if isinstance(sentence, list):
            tokens = sentence
        else:
            tokens = nltk.word_tokenize(sentence)
        ...
        noun_indices = [i for i, (token, pos) in enumerate(pos_tags) if pos in noun_pos_tags]
        ...
        for subtree in chunks.subtrees(filter=lambda t: t.label() == 'NP'):
            indices = [i for i, (token, pos) in enumerate(subtree.leaves())]
            noun_phrase_indices.extend(indices)

        target_indices = set(noun_indices).union(set(noun_phrase_indices))
