...
def load_model_and_tokenizer(cfg):
    ...
    print(f"[Model Loader] Loading base model family: {cfg.model_family} ({model_id})")
...
def save_milestone(model, tokenizer, base_dir: str, idx: int, dpath: str, split, meta: Dict, 
    ...
    if training_time_seconds is not None:
        # explicitly provided cumulative training time (preferred)
        ...
        # backward-compatible fallback: compute elapsed from start_time
        elapsed = time.time() - start_time
        meta_out["elapsed_seconds"] = float(elapsed)
        with open(os.path.join(ckpt_dir, 'running_time.txt'), 'w') as f:
            f.write(f"Total running time: {elapsed:.2f} seconds\n")
    else:
        # neither provided, write 0
        meta_out["training_time_seconds"] = 0.0
        with open(os.path.join(ckpt_dir, 'running_time.txt'), 'w') as f:
            f.write(f"Total training time (not measured): 0.00 seconds\n")
...
def main(cfg):
    ...
    for di, (dpath, dsplit) in enumerate(zip(data_paths, splits), start=1):
        ...
        print(f"[timing] Shard {di} training time: {shard_train_time:.2f} seconds. "
              f"Cumulative training time: {total_training_time:.2f} seconds")
        ...
        save_milestone(model, tokenizer, cfg.save_dir, di, dpath, dsplit, milestone_meta, start_time=None, training_time_seconds=total_training_time)
