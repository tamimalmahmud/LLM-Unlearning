...
if __name__ == "__main__":
    ...
    datasets_dir = "./CalculateL_x/retaindata"
    ...
    def unrel_shard4(i: int) -> int:
        if i < q1: return 1
        if i < q2: return 2
        if i < q3: return 3
        return 4
    UnReL_shard4 = [unrel_shard4(i) for i in range(n)]

    UnReL_shard2 = [1 if s in (1, 2) else 2 for s in UnReL_shard4]
    ...
    for s in range(4):
        ...
        for idx in indices[start:end]:
            random_shards[idx] = s + 1  

    R_shard2 = []
    for s in random_shards:
        if s in (1, 2):
            R_shard2.append(1)
        else:
            R_shard2.append(2)

#    print(f"{'file':<20} | {'probability':>12} | {'UnReL_shard':>12} | {'R_shard4':>12} | {'R_shard2':>10}")
    print(f"{'file':<20} | {'probability':>12} | {'ESU_shard2':>12} | {'ESU_shard4':>12}")
    print("-" * 80)
    for i, r in enumerate(results):
        print(
            f"{r['file']:<20} | "
            f"{r['probability']:>12.3f} | "
            f"{('S'+str(UnReL_shard2[i])):>12} | "
            f"{('S'+str(UnReL_shard4[i])):>12}" 

#            f"{('S'+str(random_shards[i])):>12} | "
#            f"{('S'+str(R_shard2[i])):>10}"
        )
