...
def load_model_and_tokenizer(cfg1):
    """
    Load model depending on whether resume_from is provided.
    If resume_from != none: load from checkpoint directory.
    Else: load base model_family.
    """
    # normalize
    resume = getattr(cfg1, "resume_from", None)
    if resume and str(resume).lower() != "none":
        print(f"[Model Loader] Loading from checkpoint: {resume}")
        tokenizer = AutoTokenizer.from_pretrained(resume)
    ...
    model_cfg = get_model_identifiers_from_yaml(cfg1.model_family)
    model_id = model_cfg["hf_key"]

    print(f"[Model Loader] Loading base model family: {cfg1.model_family} ({model_id})")
...
def save_milestone(model, tokenizer, base_dir: str, idx: int, dpath: str, split, meta: Dict,
    ...
    If folder already exists (old split), remove it before saving new data.
    """
    ...
    if training_time_seconds is not None:
        meta_out["training_time_seconds"] = float(training_time_seconds)
        with open(os.path.join(ckpt_dir, 'running_time.txt'), 'w') as f:
            f.write(f"Total training time (excluding loading/saving): {training_time_seconds:.2f} seconds\n")

    with open(os.path.join(ckpt_dir, "metadata.json"), "w", encoding="utf-8") as f:
        json.dump(meta_out, f, indent=2)

    print(f"[checkpoint] Saved shard checkpoint to: {ckpt_dir}")
    return ckpt_dir

def save_final_model(model, tokenizer, base_dir: str, forget_tag: str, cfg1,
                     start_time: Optional[float] = None,
                     total_training_time_seconds: Optional[float] = None):
    """
    Save final merged model into Final_Model_<forget>.
    Also save ONLY cfg.yaml and running_time.txt here.
    """
    final_dir = os.path.join(base_dir, f"Final_Model_{forget_tag}")

    # Remove old final folder if exists
    ...
    # Save config YAML ONLY here
    cfg_path = os.path.join(final_dir, "cfg.yaml")
    with open(cfg_path, "w") as f:
        OmegaConf.save(cfg1, f)

    # Save running time ONLY here
    if total_training_time_seconds is not None:
        with open(os.path.join(final_dir, "running_time.txt"), "w") as f:
            f.write(f"Total training time (excluding loading/saving): {total_training_time_seconds:.2f} seconds\n")
...
@hydra.main(version_base=None, config_path="config", config_name="SISA_unlearning")
def main(cfg1):
    ...
    data_paths = list(cfg1.data_paths)
    splits = list(cfg1.splits)

    # Train settings
    max_length = 350
    batch_size = cfg1.batch_size
    gradient_accumulation_steps = cfg1.gradient_accumulation_steps
    num_devices = int(os.environ.get('WORLD_SIZE', 1))
    print(f"num_devices: {num_devices}")

    model, tokenizer = load_model_and_tokenizer(cfg1)
    model.generation_config.do_sample = True

    # OSL settings (from cfg1.continual.osl)
    osl_cfg = getattr(cfg1, "continual", {}).get("osl", {})
    ...
    for di, (dpath, dsplit) in enumerate(zip(data_paths, splits), start=1):
        ...
        ds = TextDatasetQA(
            dpath, tokenizer=tokenizer, model_family=cfg1.model_family, max_length=max_length, split=dsplit
        )

        max_steps = int(cfg1.num_epochs * len(ds)) // max(1, (batch_size * gradient_accumulation_steps * num_devices))
        ...
        training_args = transformers.TrainingArguments(
            ...
            warmup_steps=max(1, max_steps // max(1, int(getattr(cfg1, 'num_epochs', 1)))),
            max_steps=max_steps,
            learning_rate=cfg1.lr,
            bf16=True,
            bf16_full_eval=True,
            logging_strategy="steps",
            logging_steps=max(1, max_steps // 100) if max_steps > 0 else 1,
            output_dir=cfg1.save_dir,
            save_strategy="steps",
        ...
        trainer.train()
        train_end = time.time()
        shard_train_time = train_end - train_start
        total_training_time += shard_train_time
        print(f"[timing] Shard {di} training time: {shard_train_time:.2f} seconds. "
              f"Cumulative training time (excluding loading/saving): {total_training_time:.2f} seconds")
        ...
        # Save milestone
        milestone_meta = {
            "model_family": cfg1.model_family,
            "num_epochs": cfg1.num_epochs,
            ...
            "shard_training_time_seconds": shard_train_time,

        }
        save_milestone(model, tokenizer, cfg1.save_dir, di, dpath, dsplit, milestone_meta, start_time=None, training_time_seconds=total_training_time)
        # If this is the LAST shard → save Final Model
        if di == len(data_paths):
            save_final_model(
                model,
                tokenizer,
                cfg1.save_dir,
    ...
    print("\n[Done] ReTraining across shards with OSL complete.")
